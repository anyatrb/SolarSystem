...
class Planet:
    ...
    Атрибуты:
    img - строка, название файла-изображения планеты без расширения
    r - целое число, радиус орбиты планеты для движения в программе
    days - целое число, количество земных суток, за которое проходит
           год на планете, для определения скорости относительно Земли
    ...
    angle - вещественное число, угол планеты на окружности в радианах
    x, y - вещественные числа, координаты планеты
    image - объект PhotoImage, файл изображения планеты
    trail - идентификатор окружности-линии орбиты планеты в Canvas
    id - идентификатор изображения планеты в Canvas
...
def init_planets():
    ...
    uranus = Planet('uranus', 390, 84*365, 0.0463, 6.8, 86.81, 25362,
                    1.27, 76, 'состоит из метана(метановой дымки),' +
                    ' ацетилена и других углеводородов', 'Уран')
    neptune = Planet('neptune', 434, 165*365, 0.00899, 5.44, 102.43, 24622,
